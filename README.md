...
* Clear a failure you have already dealt with. A failed row in the popover has a `Clear` button that turns the row and the tray icon green right away, and stays cleared across a restart until that project gets a genuinely new pipeline run. The row reads "Cleared" rather than "Passed", and nothing is changed on the CI server.
