...
* Clear a failure you have already dealt with. Hovering a failed row in the popover slides a `Clear` button in from the right (it is also reachable by keyboard), and it turns the row and the tray icon green right away, and stays cleared across a restart until that project gets a genuinely new pipeline run. The row reads "Cleared" rather than "Passed", and nothing is changed on the CI server.
