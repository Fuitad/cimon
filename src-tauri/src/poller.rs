...
mod tests {
    ...
    fn aggregate_follows_newest_commit_not_most_recently_updated() {
        // Real-world overlap (fortaress/deployment #527/#528): two pipelines on the default branch
        // for DIFFERENT commits. An older commit's pipeline (#527) finishes Success AFTER a newer
        // commit's pipeline (#528) has started, so #527 carries the LATER `updated_at`. GitLab lists
        // by `updated_at desc`, so #527 sorts FIRST even though #528 is the newer commit and is still
        // running. Picking the list head would let the settled older run mask the in-flight newer one
        // (project shows green while #528 builds). The aggregate must follow the newest commit by id.
        let mut newer_running = pipeline(528, PipelineStatus::Running);
        newer_running.sha = "f9ff6b7".into();
        newer_running.updated_at = "2026-06-26T17:03:28Z".into();
        let mut older_success = pipeline(527, PipelineStatus::Success);
        older_success.sha = "3a8f61c".into();
        older_success.updated_at = "2026-06-26T17:07:42Z".into(); // settled later -> sorts first
        let mut s = PollState::default();
