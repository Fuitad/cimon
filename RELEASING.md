...
so nothing breaks before the secrets exist. Windows and Linux are not OS-signed yet. In-app
updater artifacts are separately signed with Tauri's minisign updater key and are required on tag
releases.

## One time updater signing setup

Tauri's updater verifies every downloaded update with a minisign public key embedded in
`src-tauri/tauri.conf.json`. The matching private key must never be committed. If the private key
or its password is lost, already-installed CIMon builds cannot receive future in-app updates.

Generate the keypair once from the repo root and store the private material outside the repository:

```sh
mkdir -p ~/.config/cimon/updater-signing
chmod 700 ~/.config/cimon/updater-signing
openssl rand -base64 32 > ~/.config/cimon/updater-signing/private.key.password
chmod 600 ~/.config/cimon/updater-signing/private.key.password
npm run tauri -- signer generate --ci \
  --password "$(cat ~/.config/cimon/updater-signing/private.key.password)" \
  --write-keys ~/.config/cimon/updater-signing/private.key
```

Copy the generated `.pub` file content into `plugins.updater.pubkey` in
`src-tauri/tauri.conf.json`. Back up `private.key` and its password in a password manager or
another secure secret store before cutting the first updater-capable release.

Set the updater secrets on GitHub:

```sh
gh secret set TAURI_SIGNING_PRIVATE_KEY < ~/.config/cimon/updater-signing/private.key
...
produce signed updater artifacts, so it needs no signing secrets (`createUpdaterArtifacts` is
enabled only on tag builds).
...
For the first release that supports in-app updates, keep the cask without `auto_updates true` when
the normal `release: published` sync runs. After Homebrew has delivered that version to users once,
run the `Update Homebrew tap` workflow manually with the same tag and `enable_auto_updates=true`.
That explicit post-release path sets `auto_updates true` in `Fuitad/homebrew-tap/Casks/cimon.rb`.
Do not flip it in the same release window unless the release procedure also requires older Homebrew
installs to run `brew upgrade --greedy cimon` or `brew reinstall cimon`.
